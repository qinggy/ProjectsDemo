...
StyleSwitcher.prototype.build = function () {
    ...
    var h5 = $('<h5 />').html('样式选择').append(h5A);
    ...
    var styleSwitcherWrap = $('<div />')
        ...
        .append($('<h6 />').html('背景颜色'), colorList, $('<hr/>'));
    ...
    var fontWhite = $('<label/>').addClass('btn btn-xs btn-primary').html('白色').append(fgwbtn);
    ...
    var fontBlack = $('<label/>').addClass('btn btn-xs btn-danger').html('黑色').append(fgbbtn);
    var fgBtnGroup = $('<div/>').addClass('btn-group').attr('data-toggle', 'buttons').append(fontWhite, fontBlack);
    styleSwitcherWrap.append($('<div/>').addClass('options-link').append($('<h6/>').html('字体颜色'), fgBtnGroup));
    ...
    styleSwitcherWrap.append($('<div/>').addClass('pattern').append($('<h6/>').html('背景模式'), patternList
        )
    );
    var resetLink = $('<a/>').html('重置').attr('href', '#').on(Metis.buttonPressedEvent, function (e) {
        $this.reset();
        e.preventDefault();
    });
